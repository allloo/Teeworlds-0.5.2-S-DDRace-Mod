...
void register_update()
{
	...
	if(register_state == REGISTERSTATE_START)
	{
		...
		dbg_msg("Connecting", "refreshing ip addresses");
	}
	...
	else if(register_state == REGISTERSTATE_QUERY_COUNT)
	{
		...
		if(left == 0 || now > register_state_start+freq*3)
		{
			...
			{
				dbg_msg("Connecting", "WARNING: No master servers. Retrying in 60 seconds");
				register_new_state(REGISTERSTATE_ERROR);
			}
			else
			{			
				dbg_msg("Connecting", "choosen '%s' as master, sending heartbeats", mastersrv_name(register_registered_server));
	...
	else if(register_state == REGISTERSTATE_HEARTBEAT)
	{
		...
		{
			dbg_msg("Connecting", "WARNING: Master server is not responding, switching master");
	...
	{
		if(register_first)
			dbg_msg("Connected", "server registered");
...
int register_process_packet(NETCHUNK *packet)
{
	...
	{
		if(register_first)
			dbg_msg("Connected", "no firewall/nat problems detected");
	...
	{
		dbg_msg("register", "ERROR: configure your firewall/nat/router to let through udp on port %d.", config.sv_port);
		register_new_state(REGISTERSTATE_ERROR);
		return 1;
	}
