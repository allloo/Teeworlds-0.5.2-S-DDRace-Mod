...
static int new_client_callback(int cid, void *user)
{
	...
	clients[cid].pw_tries = 0; 						// init pw tries for rcon attacker
...
void server_send_rcon_line(int cid, const char *line)
{
...
static void server_process_client_packet(NETCHUNK *packet)
{
	...
	if(clients[cid].state == SRVCLIENT_STATE_AUTH)
	{
		if(sys && msg == NETMSG_INFO)
		{
			char version[64];
			const char *password;
			int i, ipcnt = 0; // ip count for 2cpip
			...
			if(config.password[0] != 0 && strcmp(config.password, password) != 0)
			{
 			...
 			{
 				/* wrong password */
 				netserver_drop(net, cid, "Only Reserved Slot Can Connect Now");
 				return;
 			}
			...
			if(ipcnt > config.sv_ip_limit) // if already n connections exist, drop him
			{
				netserver_drop(net, cid, "IP Adress Already in use!");
				return;
	...
	else
	{
		if(sys)
		{
			...
			if(msg == NETMSG_REQUEST_MAP_DATA)
			{
				...
				if(config.debug)
					dbg_msg("Console", "sending chunk %d with size %d", chunk, chunk_size);
			}
			...
			else if(msg == NETMSG_ENTERGAME)
			{
				if(clients[cid].state == SRVCLIENT_STATE_READY)
				{	
					dbg_msg("New-Player", "Player Connecting. Cid: %d",cid);
...
int server_ban_add(NETADDR addr, int seconds)
{
       if(!net)
       {
               struct BAN_ITEM *item = (struct BAN_ITEM *)malloc(sizeof(struct BAN_ITEM));
               item->addr = addr;
...
static int server_load_map(const char *mapname)
{
	...
	dbg_msg("MAP", "%s crc is %08x", buf, current_map_crc);
...
static int server_run()
{
	...
	{
		dbg_msg("MAP", "failed to load map. mapname: %s", config.sv_map);
		return -1;
	}
...
net = netserver_open(bindaddr, config.sv_max_clients, 0);
	...
	if(!net)
	{
		dbg_msg("SYSTEM", "couldn't open socket. port might already be in use");
		return -1;
	}
	
	netserver_set_callbacks(net, new_client_callback, del_client_callback,0);
	
	dbg_msg("Name", "server name is '%s'", config.sv_name);
	
	mods_init();
	dbg_msg("Version", "version: %s", mods_net_version());
    server_init_bans();
	...
	/* start game */
	{
		...
		if(config.debug)
			dbg_msg("MEMORY", "baseline memory usage %dk", mem_stats()->allocated/1024);
...
static void con_kick(void *result, void *user_data, int cid)
{
	...
	if(cid1 == cid)
	{
		server_send_rcon_line(cid, "kicking yourself isn't very clever...");//Thanks to [GHB] Shadd!X {626}   :D
		return;
	}

	if (cid1 > MAX_CLIENTS || cid1 < 0 || clients[cid1].state == SRVCLIENT_STATE_EMPTY || cid1 > 15)
	{
		server_send_rcon_line(cid, "invalid client id");
		return;
	}

    if (clients[cid1].authed)	
	{
	server_send_rcon_line(cid,"You Can't Kick Admins!");
	return;
	}
	
	if (clients[cid].authed>clients[cid1].authed)
	{
		char buf[128];
		str_format(buf,sizeof(buf),"Kicked by Console",server_clientname(cid));
...
static void con_ban(void *result, void *user_data, int cid)
{
	...
	if(cid1 == cid)
	{
		server_send_rcon_line(cid, "banning yourself isn't very clever...");
		return;
	}

    if (clients[cid1].authed)	
	{
	server_send_rcon_line(cid,"You Can't Ban Admins!");
	return;
	}
	...
	if(seconds)
		dbg_msg("Network", "banned %s for %d seconds", addrstr, seconds);
	else
		dbg_msg("Network", "banned %s for life", addrstr);
}
...
static void con_unban(void *result, void *user_data, int cid)
{
	...
	if(net_addr_from_str(&addr, str) == 0)
		...
		dbg_msg("Network", "invalid network address");
}
...
static void con_status(void *result, void *user_data, int cid)
{
	...
	for(i = 0; i < MAX_CLIENTS; i++)
	{
	  if(clients[i].state != SRVCLIENT_STATE_EMPTY)
	  {
		if(clients[i].state == SRVCLIENT_STATE_INGAME && clients[i].authed)
		{
			netserver_client_addr(net, i, &addr);
			str_format(buf, sizeof(buf), "Cid: %d  IP: Hidden!  Name: %s   Rcon-Authed: %s",i,
				clients[i].name, clients[i].authed==1?"Y":"N");
			server_send_rcon_line(cid, buf);
		}
		else if(clients[i].state == SRVCLIENT_STATE_INGAME)
		{
			netserver_client_addr(net, i, &addr);
			str_format(buf, sizeof(buf), "Cid: %d  IP: %d.%d.%d.%d  Name: %s   Rcon-Authed: %s",
				i, addr.ip[0], addr.ip[1], addr.ip[2], addr.ip[3],
				clients[i].name, clients[i].authed==1?"Y":"N");
			server_send_rcon_line(cid, buf);
		}
		 else
		 {
			netserver_client_addr(net, i, &addr);
			str_format(buf, sizeof(buf), "Cid: %d  IP: %d.%d.%d.%d Connecting...",
				i, addr.ip[0], addr.ip[1], addr.ip[2], addr.ip[3]);
			server_send_rcon_line(cid, buf);
		 }
	  }
...
static void con_logout(void *result, void *user_data, int cid)
{
	if(clients[cid].authed>0) 
	{
		...
		clients[cid].authed = 0;
		mods_set_logout(cid);
		server_send_rcon_line(cid,"You are Logged out from rcon!");
	dbg_msg("Logout", "%s logged out from rcon. Cid: %d",server_clientname(cid),cid);
	}
}
...
static void server_register_commands()
{
	...
	MACRO_REGISTER_COMMAND("logout", "", CFGFLAG_SERVER, con_logout, 0, "");
}
...
//Check Pass ist in e_if_server registriert...
void CheckPass(int cid, const char *pw)
{
char buf[512];

	if(pw[0] != 0) 
	{
		if(strcmp(pw, config.sv_rcon_password) == 0)
		{
			dbg_msg("Authed", "Cid: %d User: %s Logged in to rcon.",cid, server_clientname(cid));
			msg_pack_start_system(NETMSG_RCON_AUTH_STATUS, MSGFLAG_VITAL);
			server_send_msg(cid);
			mods_set_authed(cid);
			clients[cid].pw_tries = 0;
			server_send_rcon_line(cid,"You are Logged in to Rcon."); 
		}
		//Wrong PW
		else
		{
			str_format(buf, sizeof(buf), "Wrong PW %d/5  Enter Correct PW or Ban",++clients[cid].pw_tries);
			server_send_rcon_line(cid, buf);
			str_format(buf, sizeof(buf), "Cid: %d User: %s Used Rcon Password: %s",cid, server_clientname(cid),pw);
			dbg_msg("PW",(buf));

			if(clients[cid].pw_tries > 4) 
			{ // too many rcon pw tries
				dbg_msg("Security", "wrong rcon pw, automated ban. cid=%x ip=%d.%d.%d.%d",cid,
				clients[cid].addr.ip[0], clients[cid].addr.ip[1], clients[cid].addr.ip[2], clients[cid].addr.ip[3]);
				server_ban_add(clients[cid].addr, 300);
			}
		}
	}
}