...
DATAFILE *datafile_load(const char *filename)
{
	...
	dbg_msg("File", "file loading. name: %s", filename);
	...
	if(header.id[0] != 'A' || header.id[1] != 'T' || header.id[2] != 'A' || header.id[3] != 'D')
	{
		...
		{
			dbg_msg("File", "wrong signature. %x %x %x %x", header.id[0], header.id[1], header.id[2], header.id[3]);
			return 0;
		}
	}
	...
	{
		dbg_msg("File", "wrong version. version=%x", header.version);
		return 0;
	}
	...
	if(readsize != size)
	{
		dbg_msg("File", "couldn't load the whole thing, wanted=%d got=%d", size, readsize);
		return 0;
	}
	...
	if(DEBUG)
	{
		dbg_msg("File", "allocsize=%d", allocsize);
		dbg_msg("File", "readsize=%d", readsize);
		dbg_msg("File", "swaplen=%d", header.swaplen);
		dbg_msg("File", "item_size=%d", df->header.item_size);
	}
	...
	if(DEBUG)
		dbg_msg("File", "file loading done. file: %s", filename);
...
static void *datafile_get_data_impl(DATAFILE *df, int index, int swap)
{
	...
	if(!df->data_ptrs[index])
	{
		...
		if(df->header.version == 4)
		{
			...
			dbg_msg("File", "loading index=%d size=%d uncompressed=%d", index, datasize, uncompressed_size);
		...
		{
			/* load the data */
			dbg_msg("File", "loading index=%d size=%d", index, datasize);
...
int datafile_add_item(DATAFILE_OUT *df, int type, int id, int size, void *data)
{
	...
	/*
	dbg_msg("File", "added item type=%d id=%d size=%d", type, id, size);
	int i;
	for(i = 0; i < size/4; i++)
		dbg_msg("File", "\t%d: %08x %d", i, ((int*)data)[i], ((int*)data)[i]);
	*/
...
int datafile_add_data(DATAFILE_OUT *df, int size, void *data)
{
	...
	if(result != Z_OK)
	{
		dbg_msg("File", "compression error %d", result);
...
int datafile_finish(DATAFILE_OUT *df)
{
	...
	if(DEBUG)
		dbg_msg("File", "writing");

	/* calculate sizes */
	for(i = 0; i < df->num_items; i++)
	{
		if(DEBUG)
			dbg_msg("File", "item=%d size=%d (%d)", i, df->items[i].size, df->items[i].size+sizeof(DATAFILE_ITEM));
	...
	if(DEBUG)
		dbg_msg("File", "num_item_types=%d typessize=%d itemsize=%d datasize=%d", df->num_item_types, typessize, itemsize, datasize);
	...
	/* construct header */
	{
		...
		/* write header */
		if(DEBUG)
			dbg_msg("File", "headersize=%d", sizeof(header));
	...
	for(i = 0, count = 0; i < 0xffff; i++)
	{
		if(df->item_types[i].num)
		{
			...
			if(DEBUG)
				dbg_msg("File", "writing type=%x start=%d num=%d", info.type, info.start, info.num);
	...
	for(i = 0, offset = 0; i < 0xffff; i++)
	{
		if(df->item_types[i].num)
		{
			...
			{
				if(DEBUG)
					dbg_msg("File", "writing item offset num=%d offset=%d", k, offset);
	...
	{
		if(DEBUG)
			dbg_msg("File", "writing data offset num=%d offset=%d", i, offset);
		io_write(df->file, &offset, sizeof(offset));
	...
	for(i = 0; i < 0xffff; i++)
	{
		if(df->item_types[i].num)
		{
			...
			while(k != -1)
			{
				...
				if(DEBUG)
					dbg_msg("File", "writing item type=%x idx=%d id=%d size=%d", i, k, df->items[k].id, df->items[k].size);
	...
	{
		if(DEBUG)
			dbg_msg("File", "writing data id=%d size=%d", i, df->datas[i].compressed_size);
