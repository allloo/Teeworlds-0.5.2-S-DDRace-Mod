...
void engine_init(const char *appname)
{
	...
	/* */
	dbg_msg("OS", "running on %s-%s-%s", CONF_FAMILY_STRING, CONF_PLATFORM_STRING, CONF_ARCH_STRING);
#ifdef CONF_ARCH_ENDIAN_LITTLE
	dbg_msg("OS", "arch is little endian");
#elif defined(CONF_ARCH_ENDIAN_BIG)
	dbg_msg("OS", "arch is big endian");
#else
	dbg_msg("OS", "unknown endian");
#endif
...
void engine_parse_arguments(int argc, char **argv)
{
	...
	dbg_msg("Config", "paths used:");
	dbg_msg("Config", "\t%s", application_save_path);
	dbg_msg("Config", "\t%s", datadir);
	dbg_msg("Config", "saving files to: %s", application_save_path);
	...
	/* search arguments for overrides */
	{
		int i;
		for(i = 1; i < argc; i++)
			console_execute_line(argv[i],-1);
	}

	console_execute_file("autoexec.cfg");
	console_execute_file("autoexec.txt");
...
int mastersrv_refresh_addresses()
{
	...
	dbg_msg("Connecting", "refreshing master server addresses");
...
void mastersrv_update()
{
	...
	if(!needs_update)
	{
		dbg_msg("Connecting", "saving addresses");
		mastersrv_save();
	}
}
	...
	while(1)
	{
		...
		if(sscanf(line, "%s %d.%d.%d.%d", info.hostname, &ip[0], &ip[1], &ip[2], &ip[3]) == 5)
		{
			...
			if(count != MAX_MASTERSERVERS)
			{
				...
				dbg_msg("mastersrv", "warning: skipped master server '%s' due to limit of %d", line, MAX_MASTERSERVERS);
		}
		else
			dbg_msg("mastersrv", "warning: couldn't parse master server '%s'", line);
	}
