...
//	return tile == (TILE_FREEZE || TILE_BOOST_FL || TILE_BOOST_FR || TILE_BOOST_FU || TILE_BOOST_FD);
	return tile == TILE_FREEZE ||
		(tile >= TILE_BOOST_FL && tile <= TILE_BOOST_FU) ||
		(tile >= TILE_BOOST_FR && tile <= TILE_BOOST_FD);
}
...
int col_is_boost(int x, int y)
{
	...
	if	((tile >= TILE_BOOST_L && tile <= TILE_BOOST_U) ||
