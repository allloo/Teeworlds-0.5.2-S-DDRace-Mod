...
void CHARACTER::tick()
{
	if(core.jumped > 1 && core.max_jumps > core.jump_count+2)
	{
		core.jumped = 1;
		core.jump_count++;
	}
	else if (core.max_jumps == 1)
	{
		core.jumped = 2; //1 Jump
	}
	else if (core.max_jumps == 0)
	{
		core.jumped = 1; //0 Jumps
	}

	core.max_jumps = tuning.jumps;

	if(freeze_time == 0 && adminfreeze)
	{
			adminfreeze=false;
	}
	...
	if(freeze_time > 0)
	{
		...
		if (freeze_time-1==0 && !adminfreeze)
		{
			unfreeze();
		}
	}
	...
	else if(col_is_kick(pos.x, pos.y))
	{
...
bool CHARACTER::freeze(int time)
{
	...
	if (freeze_tick<server_tick()-server_tickspeed())
	{
		...
		weapons[WEAPON_NINJA].ammo = -1;//in trunk auf 0 statt -1
		last_weapon = active_weapon;
